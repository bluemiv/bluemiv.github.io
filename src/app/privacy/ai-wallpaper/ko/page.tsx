  ...
  return (
    <main>
      <div className="mx-auto max-w-[1280px] w-full flex flex-col gap-md leading-8">
        ...
        <p>
          본 개인정보 처리방침은 &lt;{appName}&gt;(이하 &apos;앱&apos;)을 운영하는 &lt;{companyName}
          &gt;이(가) 「개인정보 보호법」 제30조에 따라 정보주체의 개인정보를 보호하고 관련 고충을
          신속하고 원활하게 처리하기 위하여 수립·공개하는 지침입니다.
        </p>

        <PrivacySection title="제1조(개인정보의 처리 목적)">
          <p>
            {companyName}은(는) 다음의 목적을 위하여 최소한의 개인정보를 처리합니다. 처리하고 있는
        ...
        <PrivacySection title="제2조(처리하는 개인정보의 항목 및 수집 방법)">
          <p>
            {appName}은 별도의 회원가입 절차가 없으며, 이름, 연락처 등 개인을 식별할 수 있는 정보를
            직접 수집하지 않습니다. 서비스 이용 과정에서 아래와 같은 정보들이 자동으로 수집될 수
            있습니다.
          </p>
          <PrivacyList
            type="ol"
            items={[
              {
                ...
                extra: (
                  <div className="border-l-4 border-app-sub-bg dark:border-app-sub-bg pl-md">
                    광고 식별자는 사용자가 기기 설정에서 재설정하거나 수집을 거부할 수 있는 비영구적
                    식별자입니다.
                  </div>
                ),
              },
        ...
        <PrivacySection title="제3조(개인정보의 보유 및 이용 기간)">
          <p>
            {companyName}은(는) 별도의 서버를 통해 개인정보를 저장하지 않으므로, 앱 삭제 시 즐겨찾기
            등 모든 데이터는 사용자의 기기에서 영구적으로 삭제됩니다. 광고 목적으로 자동 수집된
            정보는 아래 수탁자의 정책에 따라 처리됩니다.
          </p>
        </PrivacySection>

        <PrivacySection title="제4조(개인정보의 제3자 제공)">
          <p>
            {companyName}은(는) 정보주체의 개인정보를 제1조(개인정보의 처리 목적)에서 명시한 범위
            내에서만 처리하며, 법률에 특별한 규정이 있는 경우를 제외하고는 정보주체의 동의 없이
            제3자에게 제공하지 않습니다.
          </p>
        </PrivacySection>
        ...
        <PrivacySection title="제5조(개인정보 처리의 위탁 및 국외 이전)">
          <p>
            {companyName}은(는) 원활한 서비스 제공을 위하여 다음과 같이 개인정보 처리업무를 국외
            사업자에게 위탁하고 있습니다.
          </p>
          <PrivacyList
            items={[
        ...
        <PrivacySection title="제6조(정보주체의 권리·의무 및 행사 방법)">
          <p>정보주체는 광고 식별자와 관련하여 다음과 같은 권리를 행사할 수 있습니다.</p>
          <PrivacyList
            items={[
              {
                title: 'Android',
        ...
        <PrivacySection title="제7조(개인정보의 파기)">
          <p>
            {companyName}은(는) 별도로 개인정보를 저장 및 관리하지 않으므로 파기 절차를 수행하지
            않습니다. 사용자가 앱을 삭제하면 모든 데이터가 파기됩니다.
          </p>
        </PrivacySection>

        <PrivacySection title="제8조(개인정보의 안전성 확보 조치)">
          <p>
            {companyName}은(는) 개인정보의 안전성 확보를 위해 기술적, 관리적 대책을 강구하고
            있습니다. 다만, 개인정보를 직접 수집·저장하지 않으므로 대부분의 조치는 광고 SDK
            제공업체인 Google의 정책을 준수합니다.
          </p>
        </PrivacySection>

        <PrivacySection title="제9조(개인정보 보호책임자)">
          <p>
            {companyName}은(는) 개인정보 처리에 관한 업무를 총괄해서 책임지고, 개인정보 처리와
            관련한 정보주체의 불만 처리 및 피해구제 등을 위하여 아래와 같이 개인정보 보호책임자를
            지정하고 있습니다.
          </p>
          <PrivacyList
            items={[
          ...
          />
          <p className="mt-sm">
            정보주체는 앱을 이용하시면서 발생한 모든 개인정보 보호 관련 문의, 불만 처리, 피해구제
            등에 관한 사항을 개인정보 보호책임자에게 문의할 수 있습니다.
          </p>
        </PrivacySection>
        ...
        <PrivacySection title="제11조(개인정보 처리방침 변경)">
          <p>
            본 개인정보 처리방침은 {startDate.format('YYYY-MM-DD')}부터 적용됩니다. 법령 및 방침에
            따른 변경내용의 추가, 삭제 및 정정이 있는 경우에는 변경사항의 시행 7일 전부터 앱 내
            공지사항 또는 웹사이트를 통하여 고지할 것입니다.
          </p>
        </PrivacySection>
      </div>
    </main>
