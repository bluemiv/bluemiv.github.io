    ...
    <main>
      <div className="mx-auto max-w-[1280px] w-full flex flex-col gap-md leading-8">
        <div className="flex gap-md justify-end">
          {[{ title: '🇺🇸 English', href: ROUTE_PATH.PRIVACY_PIXEL_BLUR_EN }].map(
            ({ title, href }) => (
              <Link
                key={title}
                href={href}
        ...
        <PrivacySection title="제2조(수집 항목 및 수집 방법)">
          <PrivacyList
            ...
            items={[
              ...
              {
                title: '수집하지 않는 항목',
                text: '이름, 연락처, 위치정보 등 직접 식별 가능한 개인정보는 앱 차원에서 수집하지 않습니다. 사용자가 갤러리에서 불러온 이미지나 편집된 결과물은 기기 내부에만 처리되며 외부 서버로 전송되지 않습니다.',
              },
            ]}
          />
        </PrivacySection>
        ...
        <PrivacySection title="제3조(보유 및 이용 기간)">
          <PrivacyList
            ...
            items={[
              ...
              {
                title: '앱 삭제 시 기기에 저장된 모든 데이터(설정 등)는 즉시 파기됩니다.',
              },
            ]}
          />
        </PrivacySection>
