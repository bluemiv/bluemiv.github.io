    ...
    <main>
      <div className="mx-auto max-w-[1280px] w-full flex flex-col gap-md leading-8">
        <div className="flex gap-md justify-end">
          {[{ title: '🇰🇷 한국어', href: ROUTE_PATH.PRIVACY_POMODORO_FLOW_KO }].map(
            ({ title, href }) => (
              <Link
                key={title}
                href={href}
            ...
            items={[
              {
                ...
                text: 'Providing Pomodoro timer, focus statistics (charts), session labeling, background timer/notifications, and settings (theme/sound) features.',
              },
              {
          ...
          <PrivacyList
            ...
            items={[
              ...
              },
              {
                ...
                text: 'Personally identifiable information such as name, contact, or location is not collected by the App. Timer data and settings are stored locally on the device and are not transmitted to external servers.',
              },
            ]}
          />
        </PrivacySection>
        ...
        <PrivacySection title="Article 3 (Retention and Use Period)">
          <PrivacyList
            ...
            items={[
              ...
              {
                title:
                  'Upon app deletion, all data stored on the device (timer history, settings, etc.) is immediately destroyed.',
              },
            ]}
          />
        </PrivacySection>
