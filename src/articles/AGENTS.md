...
- 개발자가 일상적으로 쓰는 짧은 용어(`render`, `commit`, `paint`, `state`, `props`, `client`,
  `server`)는 억지로 번역하지 않는다.
- 한 article 안에서 같은 개념의 영문, 음역, 번역 표현을 섞지 않는다. 예: `paint`, `페인트`,
  `화면 그리기`를 용어처럼 혼용하지 않는다.
...
- 일본어 article은 과도한 외래어와 직역형 수동태를 피하고, 기술 문서에서 통용되는 자연스러운
  `です/ます` 문체를 일관되게 사용한다.
- 영어 article은 짧고 명확한 technical prose와 능동태를 우선한다. 한국어 어순을 옮긴 장문,
  불필요한 `In order to`, 과도한 수동태, 같은 접속사의 반복을 피한다.
...
- 외부 참고 문서가 여러 개면 문장 안에 나열하지 않고 마지막 `## 참고 자료` section의 link
  목록으로 정리한다.
...
- 본문용 생성 이미지는 특별한 이유가 없으면 `1200×750px` WebP로 저장하고 선명도를 해치지 않는 범위에서 압축한다.
- 본문 이미지에 caption이 필요하면 공통 `ArticleFigure`를 사용한다. alt는 이미지 자체를 설명하고
  caption은 문맥과 의미를 보완하며 같은 문장을 반복하지 않는다.
- 본문 다이어그램은 구체적인 파일, browser, route와 화살표로 핵심 흐름을 바로 이해할 수 있게 한다. 의미 없는 추상 도형은 피한다.
- 공식 기술 로고와 짧은 label은 이해에 필요할 때만 사용한다. 로고 형태와 표기가 실제 product와 일치하는지 확인한다.
- 작은 장식은 넣지 않는다. 모바일에서 핵심 구조와 label을 구분할 수 있어야 한다.
