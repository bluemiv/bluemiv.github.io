...
![소스 파일이 build 엔진을 거쳐 정적 페이지와 검색, sitemap 구조로 나뉘는 과정](/r/i/nextjs/1/static-build-flow.webp)
...
![공개된 글 목록이 여러 정적 route로 생성되고 미등록 route가 제외되는 과정](/r/i/nextjs/1/static-route-generation.webp)
...
한국어 원문만 있다면 `ko.mdx`만 둬요. 번역이 없는 영어와 일본어 상세 route는 만들지 않아요. 덕분에 검색 엔진과 사용자 모두 존재하지 않는 번역 페이지로 이동하지 않아요.
...
![소스 변경, build, 검증, artifact 업로드, GitHub Pages 배포로 이어지는 과정](/r/i/nextjs/1/github-pages-deployment.webp)
