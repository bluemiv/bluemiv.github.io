...
이전 글인 [@Configuration과 설정 클래스](/articles/spring-configuration-class/)에서 `@Bean`으로 객체를 등록하는 방법을 살펴봤습니다. 이번 글에서는 등록된 Bean이 Spring 컨테이너 안에서 어떤 순서로 준비되고 정리되는지 알아보겠습니다.
...
의존성 주입 방식은 [@Autowired와 의존성 주입 방법](/articles/spring-autowired-dependency-injection/)에서 다룬 내용과 이어집니다.
...
이 방식은 [@Configuration과 설정 클래스](/articles/spring-configuration-class/)에서 다룬 외부 SDK 객체 등록과 함께 자주 사용됩니다. 직접 수정할 수 없는 외부 객체를 Spring Bean으로 등록하면서 초기화와 정리 시점까지 지정할 수 있기 때문입니다.
